...
# 3. Define the point that we wish to classify - Clearly it is Red
...
for k in range(1,number_of_classes+1):
	...
	# 6. Loop via number of points in the class -
    # # 3. Define the point that we wish to classify - Clearly it is Red
    # point_want_to_classify = [0.8,0.2]
    #
    # # **INPUT LAYER OF THE PNN **
    ...
    # 	number_of_data_point_from_class_k = len(groups.get_group(k))
    #
    # 	# ** PATTERN LAYER OF PNN **NUMBER OF POINTS IN THE CLASS!
	for i in range(1,number_of_data_point_from_class_k+1):
		...
		# 6.3 - Incrementing the row of the matrix to get the next data point
...
# Drawing the graph
fig, ax = plt.subplots()
ax.margins(0.05) # Optional, just adds 5% padding to the auto-scaling
